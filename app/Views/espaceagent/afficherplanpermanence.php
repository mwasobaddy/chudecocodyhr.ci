...
<div class="container-fluid"> 
  <!-- Page Heading -->
     ...
     <?php
		...
		///echo 'SELECT * FROM planpermanence where IDservice='.$leservice.' and IDmois =(select IDmois from lemois where numero='.$moi.' and libelle like(%'.$an.'%) ) order by creele';
...
//echo "SELECT * FROM jourplan where datejourplan like '%$dd%' order by IDjourplan<br/>";
		    ...
		    foreach ($agents as $perso)
{
	...
	//echo '<br/><br/>SELECT count(*) as nbr FROM agentplanpermanence where IDequipe IN (select IDequipe from agent_equipe where Idagent='.$myid.') and IDjourplan='.$row->IDjourplan.' and IDplanpermanence='.$idplan.'';
